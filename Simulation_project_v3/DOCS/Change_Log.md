...
## 修改 29 - 北京时间 2026/03/11 14:36

### Commit: (待提交)

**变更类型**: feat + config + docs

**变更内容**: 校准参数回写 + 补全论文第六部分6类政策场景

**受影响文件**:
- 修改: `CONFIG/mfg_config.yaml`
  - `rho`: 0.40 → 0.7284（校准值）
  - `gamma_T`: 0.30 → 0.0554（校准值）
  - `gamma_D`: 0.45 → 0.4190（校准值）
  - `gamma_W`: 0.15 → 0.2000（校准值）
- 修改: `MODULES/SIMULATOR/market_simulator.py`
  - `_adjust_mfg_params()` 扩展：新增 `target_theta_multiplier`、`alpha_T_multiplier`、`b0_multiplier` 支持
- 修改: `CONFIG/simulator_config.yaml`
  - 重写为12个场景：基准 + A~F各2档 + 组合政策
- 修改: `DOCS/项目架构与流程详细分析.md`
  - 更新SIMULATOR模块章节（5.1~5.3），反映新增场景和参数调整能力

**变更动机**:

1. 校准模块输出的参数未自动回写到 `mfg_config.yaml`，需要手动同步
2. 论文第六部分要求6类政策反事实模拟（A~F），原 `simulator_config.yaml` 仅有3个场景
3. 原 `_adjust_mfg_params` 仅支持 γ_S/γ_D 调整，无法实现场景C/D/E

**新增政策场景**:

| 场景 | 名称 | 关键参数 |
|---|---|---|
| A1/A2 | 数字素养提升(低/高) | D×1.1 / D×1.3+γ_D×1.3 |
| B1/B2 | 职业技能提升(低/高) | S×1.1 / S×1.3+γ_S×1.3 |
| C1/C2 | 就业信息优化(低/高) | θ×1.3 / θ×1.6 |
| D1/D2 | 灵活工时(低/高) | α_T×0.7 / α_T×0.5 |
| E1/E2 | 失业保障(低/高) | b0×1.5 / b0×2.0 |
| F | 组合政策 | D×1.2+S×1.2+θ×1.3 |

**影响范围**:

- MFG均衡求解和政策模拟现在使用校准后的参数值
- 模拟器支持全部6类政策场景，可直接运行 `python TESTS/test_simulator.py`
- 架构文档已同步更新
